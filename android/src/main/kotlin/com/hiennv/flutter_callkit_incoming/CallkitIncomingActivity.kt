...
class CallkitIncomingActivity : Activity() {
    ...
    private fun incomingData(intent: Intent) {
        ...
        val handle = data?.getString(CallkitConstants.EXTRA_CALLKIT_HANDLE, "") ?: ""

        val finalName = if (nameCaller.isBlank() ||
            nameCaller.equals("callkit", ignoreCase = true)
        ) {
            if (handle.isNotBlank()) handle else nameCaller
        } else {
            nameCaller
        }

        tvNameCaller.text = finalName
